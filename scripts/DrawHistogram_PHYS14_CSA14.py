...
parser.add_option("--inputList_CSA14",action="store",type="string",default="list.txt",dest="inputList_CSA14")
...
inputList_CSA14 = options.inputList_CSA14
lumi = options.lumi
...
lines_CSA14 = [line_CSA14.strip() for line_CSA14 in open(inputList_CSA14)]
...
ii = 0
for line_CSA14 in lines_CSA14:
  parts_CSA14 = line_CSA14.split()
  fileNames_CSA14.append(parts_CSA14[0])
  xsecs.append(parts[1])
  print ("dataset CSA14 : %s    xsec : %s" % (fileNames_CSA14[ii], xsecs[ii]))
...
#------- open the files CSA14 -------------
i_f = 0
for f in fileNames_CSA14:
  inf = TFile.Open(f)
  print inf.GetName()
  
  Nev = inf.Get('DijetFilter/EventCount/EventCounter').GetBinContent(1)
  print ('processed events CSA14 : %s' % Nev)
  wt = 1.0
  #if i_f < 3:

  h_allCuts_CSA14 = TH1F("h_allCuts_CSA14", "", bins, xmin, xmax)
  h_allCuts_CSA14.Sumw2()
  tree = inf.Get('rootTupleTree/tree')
  tree.Project(h_allCuts_CSA14.GetName(), var,' deltaETAjj < 1.3')
  Npassed = h_allCuts_CSA14.GetEntries()
  eff = float(Npassed)/Nev
  print('eff : %f' % eff)
  wt = LUMI*float(xsecs[i_f])*eff/Nev
  print('weight : %f' % wt)
  h_allCuts_CSA14.Scale(wt)
  h_allCuts_CSA14.Rebin(rebin)
  h_allCuts_CSA14.SetDirectory(0)
  h_allCuts_CSA14.SetLineColor(ROOT.kRed)
  h_allCuts_CSA14.SetMarkerColor(ROOT.kRed)
  hist_allCuts_CSA14.append(h_allCuts_CSA14)
...
for i in range(0,len(fileNames)) :
  ...
  hist_allCuts_CSA14[i].Scale(kFactor)


NQCD_allCuts = hist_allCuts[0].Integral()
NQCD_allCuts_CSA14 = hist_allCuts_CSA14[i].Integral()
...
hist_allCutsQCD_CSA14 = hist_allCuts_CSA14[0].Clone('hist_allCutsQCD_CSA14')

for i in range(1,len(fileNames)):
  hist_allCutsQCD.Add(hist_allCuts[i])
  hist_allCutsQCD_CSA14.Add(hist_allCuts_CSA14[i])
...
print ("bkg integral all cuts CSA14 = %f" % NQCD_allCuts_CSA14)
...
leg.AddEntry(hist_allCutsQCD_CSA14, "QCD CSA14", "l")
...
#hist_allCutsQCD_CSA14.Rebin()
hist_allCutsQCD.Draw("hist")
hist_allCutsQCD_CSA14.Draw("hist same")
leg.Draw()
#gPad.RedrawAxis()
...
ratio.Divide(hist_allCutsQCD_CSA14)
...
ratio.GetYaxis().SetTitle("#frac{PHYS14}{CSA14}")
