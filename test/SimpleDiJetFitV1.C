...
void SimpleDiJetFitV1()
{
  ...
  //########################
  // MC QCD all  
  char input_root_file[500] = "histo_signal_bkg_Dijet_MassW.root";
  //char input_directory[500] = "";
  
  char fileNameSuffix[500] = "MC"; //i.e. run period
  ...
  TH1D* hist_mass = (TH1D*) file0->Get( input_1Dhistogram );  
  hist_mass->Sumw2();
  ...
  //float bin_width_X = float ( h_DEta_Mass->GetXaxis()->GetXmax() - h_DEta_Mass->GetXaxis()->GetXmin() ) / float ( h_DEta_Mass->GetNbinsX() ) ;
    float bin_width_X =  hist_mass->GetBinWidth(1);
