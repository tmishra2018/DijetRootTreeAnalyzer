...
void analysisClass::Loop()
{
  ...
  //////////book histos here
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     Int_t no_jets_ca8=jetPtCA8->size();
     ...
     for(Long64_t ijet=0; ijet<no_jets_ca8; ijet++){ //jet loop for ca8
        if( fabs(jetEtaCA8->at(ijet)) < getPreCutValue1("jetFidRegion") && idTCA8->at(ijet) == getPreCutValue1("tightJetID")) v_idx_jet_final.push_back(ijet);
     } //end of ca8 jet loop
     ...
     if( v_idx_jet_final.size() >= 1 )
       ...
       {
         fillVariableWithValue( "pT1stJet", jetPtCA8->at(v_idx_jet_final[0]) );
         fillVariableWithValue( "eta1stJet", jetEtaCA8->at(v_idx_jet_final[0]) );
       }

     if( v_idx_jet_final.size() >= 2 )
       {
         fillVariableWithValue( "pT2ndJet", jetPtCA8->at(v_idx_jet_final[1]) );
         fillVariableWithValue( "eta2ndJet", jetEtaCA8->at(v_idx_jet_final[1]) );


         // Calculate Mjj
         jet1.SetPtEtaPhiM(jetPtCA8->at(v_idx_jet_final[0]),jetEtaCA8->at(v_idx_jet_final[0]),jetPhiCA8->at(v_idx_jet_final[0]),0);
         jet2.SetPtEtaPhiM(jetPtCA8->at(v_idx_jet_final[1]),jetEtaCA8->at(v_idx_jet_final[1]),jetPhiCA8->at(v_idx_jet_final[1]),0);
         v_jet = jet1 + jet2;
         fillVariableWithValue( "Dijet_Mass", v_jet.M() ) ;
         std::cout << " INV MASS IS " << v_jet.M() << std::endl;
         std::cout << " INV MASS FROM NTUPLE " << mjjCA8 << std::endl;
     ...
     if( v_idx_jet_final.size() >= 2) {
      ...
      h_pT1stJet->Fill(jetPtCA8->at(v_idx_jet_final[0]));
      h_pT2ndJet->Fill(jetPtCA8->at(v_idx_jet_final[1]));
      h_eta1stJet->Fill(jetEtaCA8->at(v_idx_jet_final[0]));
      h_eta2ndJet->Fill(jetEtaCA8->at(v_idx_jet_final[1]));
